...
namespace Ursine
{
    namespace ECS
    {
        ...
        bool Simulation::RayCast(RayCastOutput &output, const RayCastInput &input, const LayerMask &mask)
        {
            ...
            for (unsigned i = 0, size = _bodies.size(); i < size; ++i)
            {
                ...
                if (mask.Contains(_bodies[i]->layer) &&
                    _bodies[i]->body_type != BODY_TYPE_GHOST &&
                    _bodies[i]->_aabb.RayCast(temp_output, input))
                {
                    // And the AABB is closer to the ray start then the last result
                    if (temp_output.fraction < lowest_fraction)
                    {
